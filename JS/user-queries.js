...
const displayUserQueries =()=>{
    db.collection('queries').get().then(doc=>{
     ...
     doc.docs.map(doc=>{
        ...
        <div class="comment__details">
            <div class="name"><h2>${doc.data().name} <br><span class="email">${doc.data().email}</span></h2></div>
            <div><p></p></div>
            <div class="subject"><p>${doc.data().subject} <br><span class="comment">${doc.data().comment}</span></p></div>
                </div>
                </div>`;
